...
#if 0 // Libre disabling to support own random MACID and fix BT BLE Using same MacID
	if (hci_update_random_address(req, !connectable,
				      adv_use_rpa(hdev, flags),
				      &own_addr_type) < 0)
		return;
#endif
	own_addr_type = ADDR_LE_DEV_RANDOM; // Libre set random MACID and fix BT BLE Using same MacID
...
void __hci_req_update_adv_data(struct hci_request *req, u8 instance)
{
	...
	if (ext_adv_capable(hdev)) {
		...
		//hci_req_add(req, HCI_OP_LE_SET_ADV_DATA, sizeof(cp), &cp);	// block le adv by kernel when libre BT changes the scan property
	}
}
...
void __hci_req_update_scan(struct hci_request *req)
{
	...
	//hci_req_add(req, HCI_OP_WRITE_SCAN_ENABLE, 1, &scan);	// block set scan by kernel when libre BT changes the scan property
}
...
void __hci_req_update_class(struct hci_request *req)
{
	...
	//hci_req_add(req, HCI_OP_WRITE_CLASS_OF_DEV, sizeof(cod), cod);	// libre, blocking kernel to set class of device
}
