...
// #define GGEC_RELEASE_VERSION_CLOSE_UART_CONSOLE 1 // open this macro before MP
#define GGEC_RELEASE_VERSION_CLOSE_UART_RX 1
...
#ifdef GGEC_RELEASE_VERSION_CLOSE_UART_CONSOLE
		if (port->line != 0)
...
static void meson_receive_chars(struct uart_port *port)
{
	...
	spin_unlock(&port->lock);
#ifdef GGEC_RELEASE_VERSION_CLOSE_UART_RX
	if (port->line != 0)
...
static int meson_uart_request_port(struct uart_port *port)
{
	...
	if (port->line != 0)
		...
		val =  AML_UART_XMIT_IRQ(port->fifosize / 2);
#else
		#ifdef GGEC_RELEASE_VERSION_CLOSE_UART_RX
			if (port->line != 0)
				val = (AML_UART_RECV_IRQ(1) | AML_UART_XMIT_IRQ(port->fifosize / 2));
...
#ifndef GGEC_RELEASE_VERSION_CLOSE_UART_CONSOLE
