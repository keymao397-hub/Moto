...
static int spi_gpio_probe(struct platform_device *pdev)
{
	...
	if (!master) {
		printk("%s %d, devm_spi_alloc_master failed\n", __func__,__LINE__);
		return -ENOMEM;
	}
	...
	if (status) {
		printk("%s %d, spi_gpio_probe failed\n", __func__,__LINE__);
		return status;
	}
	...
	if (status) {
		printk("%s %d, spi_gpio_request failed\n", __func__,__LINE__);
		return status;
	}
	...
	if (status) {
		printk("%s %d, spi_bitbang_init init failed %d\n", __func__,__LINE__, status);
		return status;
	}

	status =  devm_spi_register_master(&pdev->dev, master);
	printk("%s %d, exit, status is %d\n", __func__,__LINE__, status);
	return status;
}
