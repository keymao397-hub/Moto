...
void aml_toddr_reset(struct toddr *to, int offset)
{
	unsigned int reg = 0, val = 0;

	if (!to) {
		pr_err("%s(), toddr NULL pointer\n", __func__);
		return;
	}
	...
	if (to->fifo_id == 0) {
		reg = EE_AUDIO_SW_RESET0(offset);
		val = REG_BIT_RESET_TODDRA;
	} else if (to->fifo_id == 1) {
		reg = EE_AUDIO_SW_RESET0(offset);
		val = REG_BIT_RESET_TODDRB;
	} else if (to->fifo_id == 2) {
		reg = EE_AUDIO_SW_RESET0(offset);
		val = REG_BIT_RESET_TODDRC;
	} else if (to->fifo_id == 3) {
		reg = EE_AUDIO_SW_RESET1(offset);
		val = REG_BIT_RESET_TODDRD;
	} else {
		pr_err("invalid toddr id %d\n", to->fifo_id);
		return;
	}
