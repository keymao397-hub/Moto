...
int wifi_mac_scan_for_bt(struct wlan_net_vif * wnet_vif)
{
    struct wifi_mac *wifimac = wnet_vif->vm_wmac;
    int ret_startscan =0;
    unsigned int data;
    ...
    AML_OUTPUT("%s %d start scan, flags 0x%08x, data 0x%08x\n",__func__, __LINE__, wifimac->wm_flags, data);
    ...
    ret_startscan = wifi_mac_start_scan(wnet_vif,WIFINET_SCANCFG_USERREQ|WIFINET_SCANCFG_ACTIVE| WIFINET_SCANCFG_FLUSH|WIFINET_SCANCFG_NOPICK, 0,
         NULL);
...
static int aml_inetaddr_event(struct notifier_block *this, unsigned long event, void *ptr)
{
    ...
    if (event == NETDEV_UP && wnet_vif->vm_state == WIFINET_S_CONNECTED) {
        ...
        tmp = (unsigned char *)(&ifa->ifa_address);
        AML_OUTPUT("config static IPv4 for %s: %02d.%02d.%02d.%02d", ndev->name, tmp[0],tmp[1], tmp[2],tmp[3]);
        ...
        case WIFINET_S_CONNECTED:
                ...
                && ((wnet_vif->vm_opmode == WIFINET_M_STA) && (wnet_vif->vm_flags_ext & WIFINET_FEXT_SWBMISS)))
            {
                ...
                if (!(wnet_vif->vm_flags & WIFINET_F_PRIVACY)
                    && ((wifimac->recovery_stat == WIFINET_RECOVERY_VIF_UP) || (wnet_vif->vm_use_static_ip == 1))) {
                    ...
                    if (wnet_vif->vm_use_static_ip == 1) {
                        ...
                        AML_OUTPUT("set static ip: [%d.%d.%d.%d]", ip_tmp[0], ip_tmp[1], ip_tmp[2], ip_tmp[3]);
                    }
                }
...
unsigned long last_scan_stamp = 0;
extern int wifi_mac_scan_for_bt(struct wlan_net_vif * wnet_vif);
static void wifi_mac_connect_repair_task(SYS_TYPE param1,SYS_TYPE param2, SYS_TYPE param3,SYS_TYPE param4,SYS_TYPE param5)
{
    ...
    struct wlan_net_vif *wnet_vif_temp = NULL;
    struct drv_private *drv_priv = drv_get_drv_priv();
    int ret = 0;
...
#endif
    if (aml_wifi_is_enable_rf_test()) {
        pr_info("detected rf testing mode, skip.....\n");
        return;
    }

    wnet_vif_temp = drv_priv->drv_wnet_vif_table[0];
    if (wnet_vif_temp)
    {
        if (last_scan_stamp != 0)
        {
            if (last_scan_stamp + 250 * 5 < jiffies)
            {
                ret = wifi_mac_scan_for_bt(wnet_vif_temp);
                if (ret)
                   last_scan_stamp = jiffies;
            }
        }
        else
        {
