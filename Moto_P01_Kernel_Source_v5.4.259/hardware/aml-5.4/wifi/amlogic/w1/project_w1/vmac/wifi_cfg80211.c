...
#endif
{
    ...
    if (!aml_insmod_flag) {
        pr_err("%s %d, aml insmod not complete\n",__func__,__LINE__);
        return -1;
    }
...
#endif
{
    ...
    if (sta != NULL) {
        ...
        if ((wifimac->recovery_stat == WIFINET_RECOVERY_VIF_UP) || (wnet_vif->vm_use_static_ip == 1)) {
