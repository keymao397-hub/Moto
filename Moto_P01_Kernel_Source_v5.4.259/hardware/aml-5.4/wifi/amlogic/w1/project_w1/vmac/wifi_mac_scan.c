...
struct chan_info_2g info_bt_s[14] = {0};

int convert_rssi_level(unsigned char rssi)
{
    int ret = 0;
    if (rssi > 236) {
        ret = 7;
...
void wifi_mac_update_chn_info(struct wlan_net_vif *wnet_vif) {
    struct wifi_mac *wifimac = wnet_vif->vm_wmac;
    struct wifi_mac_scan_state *ss = wifimac->wm_scan;
    struct scaninfo_table *st = ss->ScanTablePriv;
    struct scaninfo_entry *se = NULL;
    ...
    memset(info_bt_s, 0, sizeof(struct chan_info_2g) * 14);
    ...
    list_for_each_entry_safe(se, se_next, &st->st_entry, se_list) {
        ...
        if (center_chan < 14) {
            level = convert_rssi_level(rssi);
            if (level > info_bt_s[center_chan - 1].rssi_level)
            {
                info_bt_s[center_chan - 1].rssi_level = level;
                if (bw == 0)
                    info_bt_s[center_chan - 1].chan_bw = 0;
                else if (center_chan - low_chan > up_chan - center_chan)
                    info_bt_s[center_chan - 1].chan_bw = 1;
                else
                    info_bt_s[center_chan - 1].chan_bw = 2;
            }
        }

        /* AML_OUTPUT("ssid:%s, center_chan:%d, bw:%d, low_chan:%d, up_chan:%d, rssi %d\n",
            lse->SI_ssid + 2, center_chan, bw, low_chan, up_chan, rssi); */
    }
    WIFI_SCAN_SE_LIST_UNLOCK(st);

    /*************************************************************************************
    reg0: bit0-3   bit4-7   bit8-11  bit12-15  bit16-19  bit20-23  bit24-27  bit28-31
          ch1       ch2       ch3      ch4       ch5       ch6       ch7      ch8
    reg1: bit0-3   bit4-7   bit8-11  bit12-15  bit16-19  bit20-23  bit24-27  bit28-31
          ch9       ch10      ch11     ch12      ch13
    reg2: bit0-1   bit2-3   bit4-5   bit6-7    bit8-9    bit10-11  bit12-13  bit14-15
          ch1       ch2      ch3       ch4        ch5       ch6      ch7       ch8
          bit16-17 bit18-19 bit20-21 bit22-23  bit24-25
          ch9       ch10      ch11     ch12       ch13
    *************************************************************************************/
    for (i=0; i<8; i++)
    {
        reg0 |= (info_bt_s[i].rssi_level & 0x0f) << (i*4);
    }
    for (i=0; i<5; i++)
    {
        reg1 |= (info_bt_s[i+8].rssi_level & 0x0f) << (i*4);
    }
    for (i=0; i<13; i++)
    {
        reg2 |= (info_bt_s[i].chan_bw & 0x03) << (i*2);
    }
    reg2 |= 0xc0000000;
    data = wnet_vif->vif_ops.read_word(0x00f00040);
    AML_OUTPUT("wifi_mac_update_chn_info: reg0 0x%08x, reg1 0x%08x, reg2 0x%08x, data 0x%08x\n", reg0, reg1, reg2, data);

    if ((data >> 30) != 0x01)
        return ;
    wnet_vif->vif_ops.write_word(0x00f00038, reg0);
    wnet_vif->vif_ops.write_word(0x00f0003c, reg1);
    wnet_vif->vif_ops.write_word(0x00f00040, reg2);
}
...
struct wifi_channel wm_channels_2g[14] = {0};
...
static int vm_scan_setup_chan(struct wifi_mac_scan_state *ss, struct wlan_net_vif *wnet_vif)
{
    ...
    if (1)
    {
        ...
        {
            c = ss->ss_chans[i];
            if (c->chan_pri_num < 14 && c->chan_bw == 0)
            {
                map |= (1 << (c->chan_pri_num - 1));
            }
        }
		for (i=0; i<13; i++)
        {
            if ((map >> i) & 0x01 == 0)
            {
                wm_channels_2g[i].chan_cfreq1 = 2412 + (5 * i);
                wm_channels_2g[i].chan_flags = 0x4000;
                wm_channels_2g[i].chan_pri_num = i + 1;
                wm_channels_2g[i].chan_maxpower= 14;
                wm_channels_2g[i].chan_minpower = 5;
                wm_channels_2g[i].chan_bw = 0;
                wm_channels_2g[i].global_operating_class = 81;
                ss->ss_chans[ss->scan_last_chan_index++] = &wm_channels_2g[i];
            }
        }
        WIFI_CHANNEL_UNLOCK(wifimac);
    }
