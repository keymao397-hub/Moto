...
wifi_mac_add_vht_cap(unsigned char *frm, struct wifi_station *sta)
{
    ...
    if ((wnet_vif->vm_ldpc) && (wifimac->wm_flags& WIFINET_F_LDPC)) {
...
void wifi_mac_check_opt_ie(struct wlan_net_vif *wnet_vif)
{
    ...
    unsigned char ie_moved = 0;

    while (ie_oft < wnet_vif->vm_opt_ie_len) {
        ie_moved = 0;
        ie_len = frm[ie_oft + 1] + 2;
        if (ie_len + ie_oft > wnet_vif->vm_opt_ie_len) {
            ERROR_DEBUG_OUT("Malformed ies!\n");
            break;
        }
        switch (frm[ie_oft]) {
            ...
            case WIFINET_ELEMID_SUPP_REG_CLASS:
                AML_OUTPUT("remove ie id %d, tot_len %d\n", frm[ie_oft], ie_len);
                if (ie_oft + ie_len < wnet_vif->vm_opt_ie_len) {//not lastest ie
