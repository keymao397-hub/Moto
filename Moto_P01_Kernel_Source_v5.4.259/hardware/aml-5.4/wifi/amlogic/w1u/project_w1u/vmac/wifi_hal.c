...
int hal_calc_block_in_mpdu_ex(int mpdulen)
{
    int page_len = 0;
    int page_num = 0;
    struct hw_interface *hif = hif_get_hw_interface();
    page_len = hif->CommStaticParam.tx_page_len - 12;
    page_num = mpdulen / page_len + (mpdulen % page_len ? 1 : 0);
    return page_num;
}
...
{
    struct hw_interface *hif = hif_get_hw_interface();
    return howmanypage(mpdulen+FW_TXDESC_DATAOFFSET, hif->CommStaticParam.tx_page_len);
...
struct sk_buff *hal_fill_priv(struct hi_tx_priv_hdr* HI_TxPriv,unsigned char queue_id)
{
    ...
    if(aml_bus_type) {
        expand_byte = hal_calc_block_in_mpdu(HI_TxPriv->MPDULEN + HI_TXDESC_DATAOFFSET + 4) * 12;
    }
    skb = OS_SKBBUF_ALLOC(HI_TxPriv->MPDULEN + HI_TXDESC_DATAOFFSET + 4 + expand_byte);
#endif
    if (!skb)
    {
        AML_PRINT_LOG_ERR("alloc skb fail\n");
