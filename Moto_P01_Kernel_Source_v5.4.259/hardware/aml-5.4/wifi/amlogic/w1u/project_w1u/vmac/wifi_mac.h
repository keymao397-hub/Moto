...
/* wifi_mac_recovery_flags:
 *     bit0: WIFINET_RECOVERY_F_RUNNING            //set when recovery in progress
 *     bit1: WIFINET_RECOVERY_F_VIF_MAP(vif0)      //set when vif0 is running before trigger recovery
 *     bit2: WIFINET_RECOVERY_F_VIF_MAP(vif1)      //set when vif1 is running before trigger recovery
 *     bit3: WIFINET_RECOVERY_F_BSS_JOINED(vif0)   //set when vif0 is running and assoc to a BSS or start a BSS
 *     bit4: WIFINET_RECOVERY_F_BSS_JOINED(vif1)   //set when vif1 is running and assoc to a BSS or start a BSS
 *     bit5-bit7:reserved
 */

typedef unsigned char wifi_mac_recovery_flags;

#define WIFINET_RECOVERY_F (0xffff)

#define WIFINET_RECOVERY_F_RUNNING_BIT 0
#define WIFINET_RECOVERY_F_VIF_MAP_BIT_START (WIFINET_RECOVERY_F_RUNNING_BIT + 1)
#define WIFINET_RECOVERY_F_VIF_MAP_BIT_END (WIFINET_RECOVERY_F_VIF_MAP_BIT_START + WIFI_MAX_VID)
#define WIFINET_RECOVERY_F_BSS_JOINED_BIT_START (WIFINET_RECOVERY_F_VIF_MAP_BIT_END + 1)
#define WIFINET_RECOVERY_F_BSS_JOINED_BIT_END (WIFINET_RECOVERY_F_BSS_JOINED_BIT_START + WIFI_MAX_VID)

#define WIFINET_RECOVERY_F_RUNNING (BIT(WIFINET_RECOVERY_F_RUNNING_BIT))
#define WIFINET_RECOVERY_F_VIF_MASK ((BIT(WIFI_MAX_VID) - 1) << WIFINET_RECOVERY_F_VIF_MAP_BIT_START)
#define WIFINET_RECOVERY_F_VIF_MAP(_vid) (BIT(WIFINET_RECOVERY_F_VIF_MAP_BIT_START + (_vid)))
#define WIFINET_RECOVERY_F_BSS_JOINED(_vid) (BIT(WIFINET_RECOVERY_F_BSS_JOINED_BIT_START + (_vid)))

enum wifi_mac_recovery_state
{
    WIFINET_RECOVERY_INIT = 0,
    WIFINET_RECOVERY_START = 1,
    WIFINET_RECOVERY_VIF_UP = 2,
    WIFINET_RECOVERY_END,
};

enum wifi_mac_recovery_level
{
    WIFINET_RECOVERY_L_NORMAL = 0,
    WIFINET_RECOVERY_L_THOROUGH = 1,
    WIFINET_RECOVERY_L_MAX,
};

enum wifi_mac_recovery_source
{
    WIFINET_RECOVERY_SRC_NONE = 0,
    WIFINET_RECOVERY_SRC_TX_STUCK,//normal
    WIFINET_RECOVERY_SRC_TX_FAIL,
    WIFINET_RECOVERY_SRC_PAGE_ERROR,
    WIFINET_RECOVERY_SRC_WAKE_FAIL,
    WIFINET_RECOVERY_SRC_CMD_CRASH,
    WIFINET_RECOVERY_SRC_SDIO_TIMEOUT,//serious
    WIFINET_RECOVERY_SRC_MAX,
};
...
#define WIFINET_FIXED_PARAM_OFFSET 12
...
enum wifi_mac_status_code {
    ...
    WIFINET_STATUS_ANTI_TOKEN_REQUESTED = 76,
    WIFINET_STATUS_SAE_H2E = 126, //0x7e

};

enum wifi_mac_sae_message_type {
    WIFINET_SAE_COMMIT = 0x1,
    WIFINET_SAE_CONFIRM = 0x2,
};

#define WIFINET_SAE_H2E_ONLY 0xfb
enum wifi_mac_h2e_status {
    WIFINET_H2E_NONE,
    WIFINET_H2E_PEER_SUPPOR,
    WIFINET_H2E_VALID,
};
